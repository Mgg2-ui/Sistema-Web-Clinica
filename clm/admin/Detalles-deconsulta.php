...
$isread=1;
$query=mysqli_query($con,"update tblcontactos set  AdminRemark='$adminremark',IsRead='$isread' where id='$qid'");
if($query){
echo "<script>alert('Comentario de administrador actualizado correctamente.');</script>";
echo "<script>window.location.href ='Detalles-deconsulta.php'</script>";
}
		  }
?>
<!DOCTYPE html>
...
<?php
$qid=intval($_GET['id']);
$sql=mysqli_query($con,"select * from tblcontactos where id='$qid'");
$cnt=1;
